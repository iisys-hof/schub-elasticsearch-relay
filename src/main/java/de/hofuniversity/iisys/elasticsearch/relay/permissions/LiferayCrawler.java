...
public class LiferayCrawler implements IPermCrawler
{
    ...
    /**
     * Creates a new liferay crawler using the given Liferay URL, company ID
     * and retrieval credentials.
     * 
     * @param url base URL of liferay
     * @param company ID of the company the users are in
    ...
    public UserPermSet getPermissions(UserPermSet perms)
    {
        ...
        {
            // get Liferay user ID
            String response = HttpUtil.getAuthenticatedText(new URL(fLrUrl
                + USER_BY_SCR_NAME_FRAG + fCompany + SCREEN_NAME_FRAG
                + screenName), fUser, fPassword);
            ...
            // get Liferay Roles
            response = HttpUtil.getAuthenticatedText(new URL(fLrUrl
                + USER_ROLES_FRAG + userId), fUser, fPassword);
        ...
        catch (Exception e)
        {
            fLogger.log(Level.SEVERE,
                "failed to retrieve Liferay permission information.", e);
        }
