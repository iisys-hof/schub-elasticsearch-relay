...
/**
 * Filter for activties, messages and people in the Shindig index.
 * Filters by using origin and whitelist attributes and lets everyone find
 * user profiles.
 */
public class ShindigFilter implements IFilter
{
    ...
    /**
     * @param actType result type for Shindig activities
     * @param msgType result type for Shindig messages
     * @param personType result type for Shindig profiles
     */
        ...
        List<String> indices, List<String> types)
    {
        ...
        if(types.isEmpty()
            || types.contains(fActType)
            || types.contains(ESConstants.ALL_FRAGMENT))
        {
            try
            {
                originFilter = true;
