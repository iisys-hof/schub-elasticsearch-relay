package de.hofuniversity.iisys.elasticsearch.relay.permissions;
 ...
 * Collects all users and also offers a user ID lookup by Liferay ID and
 * mail address.
 */
public class PermissionCrawler implements IPermCrawler, Runnable
{
     ...
     * given Shindig URL to retrieve permissions using the given crawlers.
     * Does not start a thread on its own.
     * 
     * @param shindigUrl URL of shindig for user list retrieval
     * @param crawlers list of sub-crawlers to use
     * @param interval interval in millseconds between crawls
     * @throws Exception if intialization fails
     */
    public PermissionCrawler(String shindigUrl, List<IPermCrawler> crawlers,
    ...
    /**
     * Triggers one crawl iteration, refreshing the stored permission sets
     * using all sub-crawlers.
     * 
     * @throws Exception if crawling fails
     */
    public void crawl() throws Exception
    {
        ...
        Map<String, UserPermSet> tempPerms =
            new HashMap<String, UserPermSet>();
        ...
        if(list != null)
        {
            ...
            for(int i = 0; i < list.length(); ++i)
            {
                ...
                {
                    String mail = mailAdds.getJSONObject(j)
                        .getString(SHIND_VALUE_FIELD);
